...
See jfelten for original, unadapted helm chart. Please note it uses several things non-compatible with a k3s cluster on ARM, hence my adapted version here.
...
```bash
$ helm install -- values custom_values.yaml --name gitea --namespace gittea jzr1991/gitea
```
or locally:
...
| `ingress.annotations`          | annotations used by the ingress | `nil`                                                    |
